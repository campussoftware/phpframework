...
class Core_Pages_Render extends Core_Model_Node
{
			...
			foreach ($blockData as $blockProperties)
                        {
                            if(Core::countArray($blockProperties)>0)
                            {
                                ...
                                foreach ($blockProperties as $eachblockProperties)
                                {
                                    ...
                                    {
                                        if(Core::keyInArray('class', $eachblockProperties))
                                        {
                                            $class=$eachblockProperties['class'];
                                            if($class)
                                            {
                                                try
                                                {
                                                    ...
                                                    $block=new $class($this);   
                                                    $block->setLayout($this->_layout);
                                                    $block->setBlockName($eachblockProperties['name']);
                                                    $block->setTemplate($eachblockProperties['template']);
                                                    $block->execute();
                                                }
                                                catch (Exception $ex)
                                                {
    ...
    public function getChildBlock($blockname)
    {
        ...
        if($fileContent)
        {
            $blockdata=Core::processXmlData($fileContent,"//block[name=".$blockname."]");
            echo "<pre>";
                print_r($blockdata);
            echo "</pre>";die;
        }
